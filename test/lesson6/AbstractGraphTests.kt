...
abstract class AbstractGraphTests {
    ...
    fun longestSimplePath(longestSimplePath: Graph.() -> Path) {
        ...
        assertEquals(3, longestPath6.length)
        // end of my tests
    }
    ...
    fun baldaSearcher(baldaSearcher: (String, Set<String>) -> Set<String>) {
        ...
        // my tests
        assertEquals(setOf("A"), baldaSearcher("input/balda_in4.txt", setOf("A")))
        assertEquals(setOf(), baldaSearcher("input/balda_in4.txt", setOf("B")))
        assertEquals(
            setOf("АБСТРАКЦИЯ", "КАРТ"),
            baldaSearcher(
                "input/balda_in5.txt",
                setOf("БАЛАБОЛ", "КАРТ", "АБСТРАКЦИЯ", "ПОМЕХОУСТОЙЧИВОСТЬ", "ДИАЛЕКТИКА", "КВАЛИФИКАЦИЯ")
            )
        )
        assertEquals(
            setOf(), baldaSearcher(
                "input/balda_in6.txt", setOf(
                    "А", "Б", "В", "Г", "Д", "Е", "Ё", "Ж", "З", "И", "Й",
                    "К", "Л", "М", "Н", "О", "П", "Р", "С", "Т", "У", "Ф", "Х", "Ц", "Ч", "Ш", "Щ", "Ъ", "Э", "Ю", "Я"
                )
            )
        )
        assertEquals(
            setOf(
                "ДОЖДЬ", "ЗАТРУДНИТЕЛЬНЫЙ", "ДЕВЯТЬ", "ПРЕДДВЕРИЕ", "АДРЕНАЛИН", "АККУРАТНЫЙ", "АНАТОМИЯ",
                "БЕЗРАССУДНЫЙ", "ВДОХНОВЕНИЕ", "ВДОХ", "ГОЛОГРАММА", "ДЕЙСТВИТЕЛЬНОСТЬ", "ЕЖИК", "ИДЕАЛ",
                "ИЕРАРХИЯ", "КОМОЧЕК", "МИНУТА", "НАЧАЛО", "ОТПЕЧАТОК", "ОЦЕНКА", "ПОСТУЛАТ",
                "РАВНОВЕРОЯТНЫЙ", "ТЕНЬ", "ЭКСПЕРТ"
            ), baldaSearcher(
                "input/balda_in7.txt", setOf(
                    "ДОЖДЬ", "ЗАТРУДНИТЕЛЬНЫЙ", "ДЕВЯТЬ", "ПРЕДДВЕРИЕ", "АДРЕНАЛИН", "АККУРАТНЫЙ", "АНАТОМИЯ",
                    "БЕЗРАССУДНЫЙ", "ВДОХНОВЕНИЕ", "ВДОХ", "ГОЛОГРАММА", "ДЕЙСТВИТЕЛЬНОСТЬ", "ЕЖИК", "ИДЕАЛ",
                    "ИЕРАРХИЯ", "КОМОЧЕК", "МИНУТА", "НАЧАЛО", "ОТПЕЧАТОК", "ОЦЕНКА", "ПОСТУЛАТ",
                    "РАВНОВЕРОЯТНЫЙ", "СИМВОЛ", "ТЕНЬ", "ЭКСПЕРТ", "АГРАРНЫЙ", "ВЕРОЛОМНЫЙ", "ВРЕМЕННОЙ",
                    "ИНДИЙСКИЙ", "НАДЕРЗИТЬ", "ПЕРЕБИТЬ", "ПОЛЕСЬЕ", "ПРОИЗВЕСТИ", "СМЕНЩИК"
                )
            )
        )
        // end of my tests
